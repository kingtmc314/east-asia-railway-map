...
export const CHINA_PROVINCES = [
  ...
  { id: 'inner-mongolia-east', label: '内蒙古东', bbox: [37.0, 108.0, 50.0, 126.0] },
  ...
  { id: 'hebei', label: '河北', bbox: [36.0, 113.4, 42.6, 119.9] },
  ...
  { id: 'taiwan-mainland-claim', label: '其他', bbox: [18.0, 73.5, 53.5, 135.0] },
];
...
export function assignFeatureToProvinces(feature) {
  const fb = getFeatureBbox(feature);
  if (!fb) return ['other'];
  const [minLng, minLat, maxLng, maxLat] = fb;
  const matched = [];

  for (const province of CHINA_PROVINCES) {
    if (province.id === 'taiwan-mainland-claim') continue;
    const [s, w, n, e] = province.bbox;
    if (maxLat >= s && minLat <= n && maxLng >= w && minLng <= e) {
      matched.push(province.id);
    }
  }
...
export function manifestBbox(province) {
  const [s, w, n, e] = province.bbox;
  return [w, s, e, n]; // [west, south, east, north] for MapLibre
}
